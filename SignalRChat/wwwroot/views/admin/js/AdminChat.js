...
//admin tarafından sohbeti kiilitleme ve açma
if (chatLock) {
    chatLock.addEventListener("click", () => {
        connection.invoke("ChatLock", true).catch(function (err) { return console.error(err.toString()); });
    });
}
if (chatUnLock) {
    chatUnlock.addEventListener("click", () => {
        connection.invoke("ChatLock", false).catch(function (err) { return console.error(err.toString()); });
    });

}
//admin tarafından kullanıcıları atma
if (userOutAdmin) {
    userOutAdmin.addEventListener("click", () => {
        connection.invoke("UserOut", "Admin").catch(function (err) { return console.error(err.toString()); });
    });
}
if (userOutYetkili) {
    userOutYetkili.addEventListener("click", () => {
        connection.invoke("UserOut", "Yetkili").catch(function (err) { return console.error(err.toString()); });
    });

}
if (userOutPersonel) {
    userOutPersonel.addEventListener("click", () => {
        connection.invoke("UserOut", "Personel").catch(function (err) { return console.error(err.toString()); });
    });
